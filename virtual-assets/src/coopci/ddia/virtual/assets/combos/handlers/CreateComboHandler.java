 ...
 * 
 * */
public class CreateComboHandler extends HttpHandler {
	...
	public void service(Request request, Response response) throws Exception {
        ...
        for (String fn : request.getParameterNames()) {
        	...
        	if (segs.length == 2) {
        		...
        		} else if (segs[0].equals("item")) {
        			String va = segs[1];
        			long count = Long.parseLong(request.getParameter(fn));
        			if (va.startsWith("va_")) {
        				items.put(va, count);	
        			}
        ...
        Result res = this.engine.createCombo(uid, id, price, items, fields);
